...
user_data = [
    ...
    },
    {
        ...
        'mail': 'denise73dahoo.com',
        'name': 'Kevin Blevinffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffs',
        'website': [
...
def is_validation(user):
    ...
    if user['blood_group'] not in blood_types:
        check_title.append('blood_group')
    
    if '@' not in user['mail']:
        check_title.append('mail')
    
    if len(user['name']) < 2 or len(user['name']) > 30:
        check_title.append('name')
    
    if len(user['website']) == 0:
        check_title.append('website')

    if check_title:
        return False, check_title
    else:
        return True
...
print(f'잘못된 데이터로 구성된 유저의 수는 {incorrect_data_count} 입니다.')
